...
public class CharUtils {
	...
	public static boolean isUnreserved(char a) {
		return isAlphaNumeric(a) || a == '-' || a == '.' || a == '_'
				|| a == '~';
	}
	 ...
	 * /6/docs/api/java/net/IDN.html#toASCII%28java.lang.String,%20int%29
	 */
	...
	/**
	 * Splits a string without the use of a regex, which could split either by
	 * isDot() or %2e
	 * 
	 * @param input
	 *            the input string that will be split by dot
	 * @return an array of strings that is a partition of the original string
	 *         split by dot
	 */
	public static String[] splitByDot(String input) {
		...
		while (!reader.eof()) {
			...
			if (isDot(curr)) {
				...
				section.setLength(0);
			} else if (curr == '%' && reader.canReadChars(2)
					&& reader.peek(2).equalsIgnoreCase("2e")) {
				reader.read();
